...
ctags -R --regex-PHP='/\s*abstract\s+class\s+(\w+)/\1/' --regex-PHP='/\s*final\s+class\s+(\w+)/\1/' --regex-PHP='/\s*public\s+function\s+(\w+)/\1/' --regex-PHP='/\s*protected\s+function\s+(\w+)/\1/' --regex-PHP='/\s*interface\s+(\w+)/\1/' . /usr/share/php/
